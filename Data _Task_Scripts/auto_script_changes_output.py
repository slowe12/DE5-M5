...
def clean_date_column(df, column_name, df_name):
    if column_name in df.columns:
        ...
        if new_missing > original_missing:
            log_change("Clean Date Column", f"Converted {new_missing - original_missing} entries to NaT in '{column_name}'", df_name, len(df), len(df))
    return df
...
def convert_weeks_to_days(df, df_name):
    if "Days allowed to borrow" in df.columns:
        ...
        df["Days allowed to borrow"] = pd.to_numeric(
            df["Days allowed to borrow"].astype(str).str.split().str[0],
            errors="coerce"
        ).mul(7)
        new_na = df["Days allowed to borrow"].isna().sum()
        if new_na > original_na:
            log_change("Convert Weeks to Days", f"Converted {new_na - original_na} entries to NaN", df_name, len(df), len(df))
    return df
...
def enrich_books(df: pd.DataFrame, late_fee: float = 1.0, df_name="Books") -> pd.DataFrame:
    ...
    # Book-level metrics
    # -------------------------
    book_metrics = df.groupby("Book title").agg(
        Borrow_Count=pd.NamedAgg(column="Book title", aggfunc="count"),
        Late_Returns_Count=pd.NamedAgg(column="Was Late", aggfunc="sum"),
        Avg_Days_Borrowed=pd.NamedAgg(column="Days Borrowed", aggfunc="mean"),
        Avg_Days_Late=pd.NamedAgg(column="Days Late", aggfunc="mean")
    ).reset_index()
    
    # Merge metrics back into main df
    df = df.merge(book_metrics, on="Book title", how="left")
    
    enriched_columns = len(df.columns) - original_cols
    if enriched_columns > 0:
        log_change("Enrich Books Data", f"Added {enriched_columns} new columns including book-level metrics", df_name, original_rows, len(df), original_cols, len(df.columns), None)
...
# -------------------------
def main():
    parser = argparse.ArgumentParser(description="Clean and enrich library book checkout data.")
    ROOT = Path.cwd()
    DATA_DIR = ROOT / "data"
    ...
    print("Cleaning and enrichment complete!")
    print(f"Cleaned books saved to: {args.output_books}")
    print(f"Cleaned customers saved to: {args.output_customers}")
    print(f"Changes log saved to: {args.changes_log}")
